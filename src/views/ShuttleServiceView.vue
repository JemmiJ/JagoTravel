  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <section class="bg-gradient-to-r from-primary-600 to-primary-800 text-white py-16">
      <div class="container text-center">
        <Bus class="w-16 h-16 mx-auto mb-4" />
        <h1 class="text-4xl font-bold mb-4">Shuttle Service</h1>
        <p class="text-xl text-primary-100">Reliable airport transfers and ground transportation</p>
      </div>
    </section>

    <div class="container py-12 space-y-12">
      <BaseCard>
        <h2 class="text-2xl font-bold text-gray-900 mb-6">Book Your Shuttle</h2>
      ...
      <div>
        <h2 class="text-2xl font-bold text-gray-900 mb-8">Available Shuttle Options</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <BaseCard v-for="shuttle in shuttleOptions" :key="shuttle.name" class="overflow-hidden p-0 hover:shadow-xl transition-shadow">
            <div class="bg-gradient-to-br from-primary-500 to-primary-700 text-white p-6 text-center">
              <div class="text-5xl mb-3">{{ shuttle.icon }}</div>
              <h3 class="text-xl font-bold">{{ shuttle.name }}</h3>
              <p class="text-primary-100 text-sm">{{ shuttle.type }}</p>
            </div>
            <div class="p-6 space-y-3">
              ...
              <div class="flex items-center gap-2 text-gray-700">
                ...
                <span class="text-2xl font-bold text-primary-500">${{ shuttle.price.toLocaleString() }}</span>
      ...
      <div class="bg-gradient-to-r from-primary-600 to-primary-800 rounded-2xl shadow-lg p-8 text-white text-center">
        <h2 class="text-2xl font-bold mb-6">Why Choose Our Shuttle Service?</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div v-for="item in whyItems" :key="item.title">
            ...
            <p class="text-primary-100 text-sm">{{ item.desc }}</p>
          </div>
        </div>
      </div>
    </div>
...
import { Bus, MapPin, Clock, DollarSign, Users, Calendar } from 'lucide-vue-next'
